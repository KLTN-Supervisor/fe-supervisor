...
import { formatDate, formatHour, formatDateExtend, formatDateSpecial, formatHourExtend } from "../../../untils/format-date";
...
function ExamSchedulePage() {
  ...
  const { getBuildings, getTimes, getRooms, getSuspiciousStudents, getExamScheduleByDate } =
      ...
      for(const examSchedule of examSchedules){
        ...
        for(let i=1; i<= examSchedule.students.length; ++i) {
          table.push({
            ...
            D: examSchedule.students[i-1].student.last_name +
              " " +
        ...
        const worksheet = XLSX.utils.json_to_sheet([], { origin: 'B20', skipHeader: true });
        XLSX.utils.book_append_sheet(workbook, worksheet, `${examSchedule.room_name}_${formatHourExtend(examSchedule.start_time)}`);
      }
    ...
    const addStyles = (workbookBlob, finalData, examSchedules) =>{
      return xlsxPopulate.fromDataAsync(workbookBlob).then((workbook) => {
        workbook.sheets().forEach((sheet, index) => {
          ...
          title2.value(`Học kỳ 0${examSchedules[index].term} - Năm học ${examSchedules[index].year.from}-${examSchedules[index].year.to}`);
          ...
          subjectValue.value(`${examSchedules[index].subject_name} - Số Tín Chỉ: ${examSchedules[index].subject_credit}`);
          ...
          examDateValue.value(`${formatDate(examSchedules[index].start_time)} - Giờ Thi: ${formatHour(examSchedules[index].start_time)}  -   phút - Số Tiết 2 - Phòng thi: ${examSchedules[index].room_name}`);
          ...
          for(let i=1; i<finalData[index].length; ++i){
            ...
            sheet.cell(`AI${i+20}`).style({
              border: true,
              fill: finalData[index][i].L === "Vắng thi" ? (
                {type: "solid",
                color: {
                    rgb: "ffff00"
                }
          ...
          const studentTotal = sheet.range(`C${23+finalData[index].length}:G${25+finalData[index].length}`).merged(true).style({
            fontSize: 10,
            fontFamily: "Times New Roman",
            verticalAlignment: "center",
            horizontalAlignment: "center",
          ...
          const studentTotalValue = sheet.range(`H${23+finalData[index].length}:H${25+finalData[index].length}`).merged(true).style({
            fontSize: 10,
            fontFamily: "Times New Roman",
            verticalAlignment: "center",
            horizontalAlignment: "center",
          ...
          const studentTotalAttend = sheet.range(`I${22+finalData[index].length}:L${25+finalData[index].length}`).merged(true).style({
            fontSize: 10,
            fontFamily: "Times New Roman",
            verticalAlignment: "center",
            wrapText: true
          ...
          const studentTotalAttendValue = sheet.range(`M${22+finalData[index].length}:P${25+finalData[index].length}`).merged(true).style({
            fontSize: 10,
            fontFamily: "Times New Roman",
            verticalAlignment: "center",
            wrapText: true,
          ...
          studentTotalAttendValue.value(`   ${countObjectsWithAttendanceTrue(finalData[index])}   `);
  
          const date = sheet.range(`X${23+finalData[index].length}:Z${25+finalData[index].length}`).merged(true).style({
            fontSize: 10,
            fontFamily: "Times New Roman",
            verticalAlignment: "center",
            horizontalAlignment: "center",
          ...
          date.value("Ngày");
          const month = sheet.range(`AB${23+finalData[index].length}:AD${25+finalData[index].length}`).merged(true).style({
            fontSize: 10,
            fontFamily: "Times New Roman",
            verticalAlignment: "center",
            horizontalAlignment: "center",
          ...
          month.value("tháng");
          const year = sheet.range(`AF${23+finalData[index].length}:AH${25+finalData[index].length}`).merged(true).style({
            fontSize: 10,
            fontFamily: "Times New Roman",
            verticalAlignment: "center",
            horizontalAlignment: "center",
          ...
          year.value("năm");
          const signSubject = sheet.range(`C${27+finalData[index].length}:J${27+finalData[index].length}`).merged(true).style({
            fontSize: 10,
            fontFamily: "Times New Roman",
            verticalAlignment: "center",
            horizontalAlignment: "center",
          ...
          const signInpector = sheet.range(`V${27+finalData[index].length}:AI${27+finalData[index].length}`).merged(true).style({
            fontSize: 10,
            fontFamily: "Times New Roman",
            verticalAlignment: "center",
            horizontalAlignment: "center",
